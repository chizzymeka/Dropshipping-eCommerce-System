...
<div id="adminMenu" class="alignLeft">
    ...
    <br>
    <p align="center"><a href="<c:url value='shipperLoginForm'/>"><strong>Log Out</strong></a></p>
</div>

<%-- readyToBeShippedOrdersList is requested --%>
<c:choose>
    <c:when test="${!empty readyToBeShippedOrdersList} || ${shipperStatus eq true}">
    ...
    <c:otherwise>
        ...
        <c:if test="${!empty noAvailableOrdersErrorMessage}">
            <script type="text/javascript">
                alert("<%=request.getAttribute("noAvailableOrdersErrorMessage")%>");
            </script>
        </c:if>
    </c:otherwise>
</c:choose>
...
<%-- associatedSuppliersList is requested --%>
